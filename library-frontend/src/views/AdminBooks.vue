<template>
    ...
    <v-container style="height: 100%" grid-list-md>
     ...
     <v-data-table :headers="headers" :items="books" class="elevation-1 mytable" no-data-text="No se encontraron libros"
        :footer-props="{
...
export default{
    ...
    computed: {
        ...
        aboutConfirm(){
            return this.dialog === true ? 'Esta seguro que desea actualizar la cantidad de ejmplares de ' + this.title + '?' : 'Esta seguro que desea eliminar ' + this.itemDelete.titulo + '?'
        },
        titleConfirm(){
            return this.dialog === true ? 'Actualizar cantidad de ejmplares' : 'Eliminar libro'
        },
    },
    ...
    methods: {
        ...
        addBook(){
            this.$axios.post('http://localhost:8080/libros' , {
                        titulo: this.title,
        ...
        changeAmountCopies(){
            this.$axios.put('http://localhost:8080/libros/' + this.editedItem.id_libro, {
                    cantidad: parseInt(this.amount)
                })
                    .then((data) => {
                        this.getBooks();
        ...
        getAmountCopiesAvailables(item){
            this.$axios.get('http://localhost:8080/libros/' + item.id_libro)
                    .then((data) => {
        ...
        ,
        deleteBook(idLibro){
            this.$axios.delete('http://localhost:8080/libros/' + idLibro)
                    .then(() => {
                        this.getBooks();
...
/* .imagen {
  background-image: linear-gradient(
    ...
    url("../assets/antique_library_desk-wallpaper-1920x1080.jpg");
